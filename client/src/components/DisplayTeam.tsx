...
export function DisplayTeam(
    ...
    Object.entries(displayTeam.team).map(pl => pl[1]).sort((a, b) => a.position - b.position).forEach((player) => {
            ...
            <div className='player-display-name'>
              <div>{player['web_name']}</div>
              {
                ...
                ? <div
                    ...
                    onDrop={(event) => onDragCaptainDropCallback(event)}>C</div> 
                :(player['is_vice_captain'] ?
                  <div
                    ...
                    onDragOver={(event) => onDragCaptainOverCallback(event)}
                    onDragLeave={(event) => onDragCaptainLeaveCallback(event)}
                    onDrop={(event) => onDragCaptainDropCallback(event)}>V</div>
                : <div
                    ...
                    onDragOver={(event) => onDragCaptainOverCallback(event)}
                    onDragLeave={(event) => onDragCaptainLeaveCallback(event)}
                    onDrop={(event) => onDragCaptainDropCallback(event)}>-</div>)
              }
            </div>
